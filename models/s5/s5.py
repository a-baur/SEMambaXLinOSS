...
class S5SSM(torch.nn.Module):
    def __init__(
        ...
        match bcInit:
            ...
            case "factorized":
                print(
                    "[WARN]: factorized was removed from the original repo, might be for a reason :?"
                )
                # Use a low rank factorization of rank k for B and C
                self.BH = torch.nn.Parameter(
                    as_complex(init_columnwise_B((h, k, 2), torch.float32))
                )
                self.BP = torch.nn.Parameter(
                    as_complex(init_columnwise_B((p, k, 2), torch.float32))
                )
    ...
    def forward(
        self, signal, step_scale: float | torch.Tensor = 1.0, state=None, return_state=False
    ):
        B_tilde, C_tilde = self.get_BC_tilde()
        ...
        assert not (self.bidir and (state is not None)), (
            "injecting state is not compatible with bidirectional S5"
        )
        ...
        assert not (self.bidir and return_state), (
            "return_state does not work with bidirectional S5"
        )
        if return_state:
            return out, state
        return out
...
class S5(torch.nn.Module):
    ...
    def forward(
        ...
        if state is None:
            return torch.vmap(lambda s, ss: self.seq(s, step_scale=ss, return_state=return_state))(
                signal, step_scale
            )
        else:
            return torch.vmap(
                lambda s, ss, _state: self.seq(
                    s, step_scale=ss, state=_state, return_state=return_state
                )
            )(signal, step_scale, state)
