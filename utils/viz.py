...
def log_audio_and_spectrograms(sw, idx, steps, sr, hop_size, compress_factor,
                               ...
                               log_reference=True, max_seconds=None):
    ...
    the figures readable for long utterances.
    """
    ...
    if log_reference:
        ...
        clean_fig = _spectrogram_figure(clean_mag.squeeze(), compress_factor, sr, hop_size, "Clean", max_frames)
        noisy_fig = _spectrogram_figure(noisy_mag.squeeze(), compress_factor, sr, hop_size, "Noisy", max_frames)
    ...
    enhanced_fig = _spectrogram_figure(enhanced_mag.squeeze(), compress_factor, sr, hop_size, "Enhanced", max_frames)
...
def _spectrogram_figure(mag, compress_factor, sr, hop_size, title="", max_frames=None):
    ...
    `max_frames`, if given, clips the time axis to that many STFT frames.
    """
